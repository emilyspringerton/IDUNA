...
Weapons (category "weapon") also need a real "delay" field -- attack speed in delay-units, 60 du ≈ 1 second. This is an INDIVIDUAL, per-weapon value, not a fixed constant per weapon type: two swords can and should have different delays from each other based on their own real weight/design (a heavier or more ornate weapon of the same type is often slightly slower, not identical to every other sword). Rough real reference points to vary around, not copy exactly: daggers ~150-190, one-handed swords ~200-260, axes ~260-300, great swords/heavy two-handed ~400-500, staves ~330-380, bows ~260-300. Omit "delay" entirely for non-weapon categories.

Given this item name: %q -- propose a real, plausible item definition as JSON matching this exact schema: {"name":string,"category":string,"level":int,"equip_slots":[string],"jobs":[string],"stats":{string:int},"stack_size":int,"delay":int,"model_id":string,"description":string}. Return ONLY the JSON object, no markdown fences, no commentary.`, minLevel, maxLevel, itemName)
