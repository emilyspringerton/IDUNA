...
func TestPapercraftTicket_MintsValidTicket(t *testing.T) {
	...
	secret := []byte("test-papercraft-shared-secret")
	playerID := uuid.New()
	token := makeAgentToken(t, keys, playerID.String(), nil)

	h := papercraftTicketHandlerWithAuth(keys, secret)
	...
	wantExpiryWindow := time.Now().Add(handlers.PapercraftTicketTTL)
...
func TestPapercraftTicket_RequiresAuth(t *testing.T) {
	keys, _ := jwt.GenerateKeys()
...
func TestPapercraftTicket_RejectsNonUUIDSubject(t *testing.T) {
	keys, _ := jwt.GenerateKeys()
...
func TestPapercraftTicket_RequiresSecretConfigured(t *testing.T) {
	keys, _ := jwt.GenerateKeys()
	token := makeAgentToken(t, keys, uuid.New().String(), nil)

	h := papercraftTicketHandlerWithAuth(keys, nil) // no PAPERCRAFT_TICKET_SECRET set
	req := httptest.NewRequest(http.MethodPost, "/api/v1/papercraft/ticket", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
