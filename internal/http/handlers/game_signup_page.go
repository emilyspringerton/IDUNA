// GameSignupPageHandler is a plain, public, unauthenticated web page that drives the generic
// guest-account API (game_online.go's guest-register/guest-login/guest-upgrade) for one
// internal/games.Registry entry. Built for BIG_O (kanban card 123214231: "we need a big_o
// account creation interface off of iduna" -- BIG_O has no native client yet, NORTHSTAR only) and
// immediately reused for BRAWLPIT (SHANKPIT_OS_NORTHSTAR.md item 4: "BRAWLPIT has no player-
// facing IDUNA identity integration at all") rather than copy-pasted per game -- the SAME real
// "one row, not a new handler" discipline internal/games.Registry itself already follows, one
// level up at the page layer. Any future games.Registry entry that also has no native client yet
// gets this same page for free by mounting one more route.
package handlers
...
type GameSignupPageHandler struct {
	// Game is the internal/games.Registry slug (e.g. "big_o", "brawlpit") -- used verbatim in
	// the API paths this page calls (/api/v1/games/{Game}/guest-*) and as the localStorage key
	// namespace, so two games' saved player ids on the same browser never collide.
	Game string
	// Title is the big display heading (e.g. "BIG_O", "BRAWLPIT").
	Title string
	// Tagline is the one-line subtitle under Title (e.g. "A SHANKPIT Story — account creation").
	Tagline string
}

func (h *GameSignupPageHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// html/template auto-escapes Game/Title/Tagline into their HTML/JS contexts -- these three
	// fields are only ever set by main.go's own route wiring (never user input), but there's no
	// real cost to using the safe template engine here over a raw string anyway.
	_ = gameSignupPageTmpl.Execute(w, h)
}

var gameSignupPageTmpl = template.Must(template.New("game-signup").Parse(`<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<script>
const GAME = {{.Game}};
const API = '/api/v1/games/' + GAME;
const STORAGE_KEY = GAME + '_player_id';
...
function onLoggedIn(playerId, token, displayName) {
  ...
  try { localStorage.setItem(STORAGE_KEY, playerId); } catch (e) {}
}
