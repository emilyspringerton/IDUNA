...
func TestCreateMaterial_DefaultsShaderNameToStandard(t *testing.T) {
	...
	m, err := s.CreateMaterial(context.Background(), "glass", "", nil, 0.5, 32)
	if err != nil {
...
func TestCreateMaterial_RejectsUnknownShader(t *testing.T) {
	s := newTestMaterialStore(t)
	if _, err := s.CreateMaterial(context.Background(), "glowstone", "flourescent_light", nil, 0, 8); err == nil {
...
func TestCreateMaterial_RejectsOutOfRangeShading(t *testing.T) {
	s := newTestMaterialStore(t)
	if _, err := s.CreateMaterial(context.Background(), "glass", "", nil, 1.5, 32); err == nil {
		t.Fatal("expected an error for specular above the real maximum")
	}
	if _, err := s.CreateMaterial(context.Background(), "glass", "", nil, 0.5, 0); err == nil {
...
func TestGetMaterialByName_RealLookup(t *testing.T) {
	s := newTestMaterialStore(t)
	if _, err := s.CreateMaterial(context.Background(), "metal", "", nil, 0.85, 64); err != nil {
		t.Fatalf("create: %v", err)
	}
...
func TestUpdateMaterial_SetsTextureOverride(t *testing.T) {
	s := newTestMaterialStore(t)
	created, err := s.CreateMaterial(context.Background(), "wood", "", nil, 0.06, 8)
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	texID := int64(42)
	updated, err := s.UpdateMaterial(context.Background(), created.ID, "", &texID, 0.06, 8)
	if err != nil {
...
func TestDeleteMaterial(t *testing.T) {
	s := newTestMaterialStore(t)
	created, err := s.CreateMaterial(context.Background(), "concrete", "", nil, 0.03, 4)
	if err != nil {
		t.Fatalf("create: %v", err)
	}
