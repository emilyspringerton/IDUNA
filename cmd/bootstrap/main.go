...
// writeSecretsEnv writes secrets to path, MERGING with any plaintext
// entries already in the file rather than replacing it outright.
//
// Found live (S141-04, registering the NORN agent): on a run where only
// some agents need a credential (the common case — most runs provision
// zero new agents, but this one provisioned 4), `secrets` contains only
// the newly-generated ones. The previous version of this function wrote
// exactly that slice and nothing else, silently destroying the plaintext
// for every previously-provisioned agent not in this run's `secrets` —
// the database's api_key_hash was untouched (so those agents still work),
// but their plaintext was gone from the only place it's ever written
// (this file is git-ignored, generated once, never backed up elsewhere by
// design). Recovered EMILY-PRIME's from a live process's environment;
// FATBABY-EMILY/EMIREE/JON/BOB/TYLER's were not recoverable and had to be
// rotated. Fixed here so this can't happen again: existing `export
// IDUNA_SECRET_X=...` lines are parsed and preserved for any agent not in
// the current secrets slice; secrets do carry over across runs.
func writeSecretsEnv(path string, secrets []agentSecret) error {
	// Everything is keyed by the env-var form (upper snake case) throughout,
	// including entries recovered from the existing file — the actual
	// `export KEY=value` lines are never round-tripped through a hyphen
	// guess, which would be lossy and is exactly the kind of subtlety
	// worth avoiding in a credentials file. The human-readable "Agent %s:
	// set..." comment is cosmetic only and best-effort for entries
	// preserved from disk (real name known in this run's `secrets` slice;
	// reconstructed from the env key otherwise by stripping the
	// IDUNA_SECRET_ prefix and swapping _ back to - — correct for every
	// agent name in this codebase today, none of which contain a literal
	// underscore, but not guaranteed in general).
	type entry struct {
		envKey      string
		plaintext   string
		displayName string
	...
	for envKey, plaintext := range readExistingSecretLines(path) {
	...
	for _, s := range secrets {
		...
		merged[envKey] = &entry{envKey: envKey, plaintext: s.Plaintext, displayName: s.Name}
	}
	sort.Strings(order)
...
// readExistingSecretLines parses `export IDUNA_SECRET_<KEY>=<value>` lines
// from an existing secrets file, returning envKey (e.g.
// "IDUNA_SECRET_EMILY_PRIME") -> plaintext. Returns an empty map if the
// file doesn't exist or can't be parsed; never an error, since a
// missing/corrupt file just means nothing to merge, not a failure.
func readExistingSecretLines(path string) map[string]string {
	out := map[string]string{}
	data, err := os.ReadFile(path)
	if err != nil {
		return out
	}
	re := regexp.MustCompile(`^export (IDUNA_SECRET_[A-Z0-9_]+)=(\S+)$`)
