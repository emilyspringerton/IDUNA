...
| GET | `/play/big_o` | Public BIG_O account-creation page (no native BIG_O client exists yet) — drives the generic `internal/games.Registry` guest-register/guest-login/guest-upgrade API, same code path DEADWEIGHT's own native client already uses. |
